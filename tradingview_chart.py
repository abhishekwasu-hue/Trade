...
def _price_binned_profile(df, value_column, num_bins=24):
    """
    🎓 सामायिक (shared) गणना — किंमतीनुसार (price-binned) horizontal histogram, कुठल्याही per-candle
    संख्यात्मक स्तंभासाठी (volume किंवा oi). वेळेनुसार नाही (जसं खालचा नेहमीचा Volume pane दाखवतो तसं
    नाही). आपल्याकडे tick-data नाही, फक्त candle-level OHLCV(+OI) आहे — म्हणून प्रत्येक candle चं मूल्य,
    त्याच्या high-low range मध्ये येणाऱ्या सगळ्या price-bins मध्ये समान वाटून (proportional
    approximation) टाकलं आहे. compute_volume_profile()/compute_oi_profile() या दोन्ही याच core वर
    बांधलेले आहेत.

    रिटर्न: [{"price_low", "price_high", "value", "is_poc"}, ...] — फक्त शून्यापेक्षा जास्त मूल्य
    असलेले bins (is_poc = सर्वात जास्त मूल्य असलेला bin).
    """
    if df is None or df.empty or value_column not in df.columns or num_bins <= 0:
        return []
    ...
    max_val = max(bins) if bins else 0.0
    profile = []
...
def compute_volume_profile(df, num_bins=24):
    """
    🎓 Volume Profile — किंमतीनुसार (price-binned) horizontal volume histogram.

    रिटर्न: [{"price_low", "price_high", "volume", "is_poc"}, ...] (is_poc = Point of Control —
    सर्वात जास्त volume असलेला bin).
    """
    return [
        {"price_low": b["price_low"], "price_high": b["price_high"], "volume": b["value"], "is_poc": b["is_poc"]}
        for b in _price_binned_profile(df, "volume", num_bins)
    ]


def compute_oi_profile(df, num_bins=24):
    """
    🎓 OI Profile — Volume Profile सारखंच, पण Open Interest वर आधारित (Upstox च्या historical-candle
    API मधून प्रत्येक candle सोबतच "oi" येतो — NIFTY/BANKNIFTY/SENSEX व MCX Futures दोन्हीकडच्या
    df मध्ये आधीपासूनच उपलब्ध, वेगळा API कॉल लागत नाही). Index instruments साठी OI शून्य असू शकतो —
    तेव्हा profile रिकामाच राहतो (त्रुटी नाही, फक्त काही दाखवण्यासारखं नाही).

    रिटर्न: [{"price_low", "price_high", "oi", "is_poc"}, ...] (is_poc = सर्वात जास्त OI असलेला bin).
    """
    return [
        {"price_low": b["price_low"], "price_high": b["price_high"], "oi": b["value"], "is_poc": b["is_poc"]}
        for b in _price_binned_profile(df, "oi", num_bins)
    ]
...
def build_lightweight_chart_html(
    ...
    असतील तेच) + Volume Profile व OI Profile (किंमतीनुसार horizontal bars, प्रत्येकाच्या स्वतंत्र
    toolbar बटणाने on/off — डीफॉल्ट दोन्ही बंद) + Drawing Toolbar (Trendline, Horizontal Line,
    Fibonacci, Rectangle, Measure).
    ...
    oi_profile_data = compute_oi_profile(df)
...
// --- OI Profile: Volume Profile सारखंच, पण Open Interest वर आधारित — उजवीकडून सुरू होणाऱ्या bars
// (Volume Profile डावीकडून सुरू होतो), जेणेकरून दोन्ही एकाच वेळी चालू असतील तरी गोंधळ होणार नाही.
class OIProfileRenderer {{
    constructor(source) {{ this._source = source; }}
    draw(target) {{
        const data = this._source._data;
        const series = this._source._series;
        ...
        target.useBitmapCoordinateSpace(scope => {{
            const ctx = scope.context;
            const maxOi = Math.max(...data.map(d => d.oi));
            if (!maxOi) return;
            const paneWidth = scope.bitmapSize.width;
            const maxBarWidth = paneWidth * 0.25;
            ctx.save();
            data.forEach(d => {{
                const yTop = series.priceToCoordinate(d.price_high);
                const yBottom = series.priceToCoordinate(d.price_low);
                ...
                const yBottomPx = yBottom * scope.verticalPixelRatio;
                const barHeight = Math.max(1, yBottomPx - yTopPx - 1);
                const barWidth = (d.oi / maxOi) * maxBarWidth;
                ctx.fillStyle = d.is_poc ? 'rgba(255,64,129,0.55)' : 'rgba(171,71,188,0.28)';
                ctx.fillRect(paneWidth - barWidth, yTopPx, barWidth, barHeight);
            }});
            ctx.restore();
        }});
    }}
}}
class OIProfilePaneView {{
    constructor(source) {{ this._source = source; }}
    renderer() {{ return new OIProfileRenderer(this._source); }}
}}
class OIProfilePrimitive {{
    constructor(data) {{
        this._data = data;
        this._paneViews = [new OIProfilePaneView(this)];
    }}
    updateAllViews() {{}}
    paneViews() {{ return this._paneViews; }}
    attached({{ series }}) {{ this._series = series; }}
}}
const oiProfileData = {json.dumps(oi_profile_data)};
let oiPrimitive = null;
function toggleOIProfile() {{
    const btn = document.getElementById('btn_oiprofile');
    if (oiPrimitive) {{
        candleSeries.detachPrimitive(oiPrimitive);
        oiPrimitive = null;
        btn.classList.remove('active');
    }} else if (oiProfileData.length > 0) {{
        oiPrimitive = new OIProfilePrimitive(oiProfileData);
        candleSeries.attachPrimitive(oiPrimitive);
        btn.classList.add('active');
    }}
}}
