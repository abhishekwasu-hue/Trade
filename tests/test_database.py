...
class TestCheckBreakoutBuildup:
    """🎓 वापरकर्त्याशी चर्चा करून ठरवलेली सुधारणा (Breakout Entry — "Max 2 trade on same level hit,
    he honar donhi sl or tsl hit jhalet, ani nantr jar Breakout buildup ... then take entry") —
    "buildup" = त्याच level वर आज किमान 2 CLOSED trades, आणि सगळे SL/TSL लागून हरलेले."""

    def test_less_than_two_closed_trades_returns_false(self, temp_db):
        seed_closed_trade(temp_db, "T1", -500.0, "SL", "2026-09-24", entry_level_price=23900.0)
        assert database.check_breakout_buildup("NIFTY", 23900.0, "2026-09-24") is False

    def test_two_sl_trades_returns_true(self, temp_db):
        seed_closed_trade(temp_db, "T1", -500.0, "SL", "2026-09-24", entry_level_price=23900.0)
        seed_closed_trade(temp_db, "T2", -300.0, "TSL_SL", "2026-09-24", entry_level_price=23900.0)
        assert database.check_breakout_buildup("NIFTY", 23900.0, "2026-09-24") is True

    def test_one_target_exit_returns_false(self, temp_db):
        """level निदान एकदा तरी टिकला (Target लागला) -- buildup नाही."""
        seed_closed_trade(temp_db, "T1", -500.0, "SL", "2026-09-24", entry_level_price=23900.0)
        seed_closed_trade(temp_db, "T2", 800.0, "TARGET", "2026-09-24", entry_level_price=23900.0)
        assert database.check_breakout_buildup("NIFTY", 23900.0, "2026-09-24") is False

    def test_different_level_price_not_counted(self, temp_db):
        seed_closed_trade(temp_db, "T1", -500.0, "SL", "2026-09-24", entry_level_price=23900.0)
        seed_closed_trade(temp_db, "T2", -300.0, "SL", "2026-09-24", entry_level_price=24000.0)
        assert database.check_breakout_buildup("NIFTY", 23900.0, "2026-09-24") is False

    def test_different_source_not_counted(self, temp_db):
        seed_closed_trade(temp_db, "T1", -500.0, "SL", "2026-09-24", entry_level_price=23900.0)
        seed_closed_trade(temp_db, "T2", -300.0, "SL", "2026-09-24", entry_level_price=23900.0, source="srv2_momentum_reversal")
        assert database.check_breakout_buildup("NIFTY", 23900.0, "2026-09-24") is False

    def test_different_day_not_counted(self, temp_db):
        seed_closed_trade(temp_db, "T1", -500.0, "SL", "2026-09-23", entry_level_price=23900.0)
        seed_closed_trade(temp_db, "T2", -300.0, "SL", "2026-09-24", entry_level_price=23900.0)
        assert database.check_breakout_buildup("NIFTY", 23900.0, "2026-09-24") is False

    def test_still_open_trade_not_counted(self, temp_db):
        """CLOSED नसलेला (अजून OPEN) trade मोजला जायला नको -- फक्त 1 CLOSED असेल तर False."""
        seed_closed_trade(temp_db, "T1", -500.0, "SL", "2026-09-24", entry_level_price=23900.0)
        conn = sqlite3.connect(temp_db)
        conn.execute(
            """INSERT INTO live_trades (trade_id, trade_date, symbol, strategy, entry_time, status,
               legs_json, mode, trading_style, source, entry_level_price) VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            ("T2", "2026-09-24", "NIFTY", "BEAR_CALL_SPREAD", "2026-09-24 11:00:00", "OPEN",
             json.dumps([]), "PAPER", "INTRADAY", "dynamic_sr_instant", 23900.0),
        )
        conn.commit()
        conn.close()
        assert database.check_breakout_buildup("NIFTY", 23900.0, "2026-09-24") is False

    def test_three_closed_trades_all_sl_still_true(self, temp_db):
        """>=2 (नेमकं 2 नाही) पण सगळे SL/TSL असतील तरी buildup खरं -- उदा. spread+naked दोन्ही touches चे."""
        seed_closed_trade(temp_db, "T1", -500.0, "SL", "2026-09-24", entry_level_price=23900.0)
        seed_closed_trade(temp_db, "T2", -300.0, "TSL_SL", "2026-09-24", entry_level_price=23900.0)
        seed_closed_trade(temp_db, "T3", -200.0, "SL", "2026-09-24", entry_level_price=23900.0)
        assert database.check_breakout_buildup("NIFTY", 23900.0, "2026-09-24") is True
